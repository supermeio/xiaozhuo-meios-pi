...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 13. Stream responses through if streaming, otherwise buffer
    const respContentType = upstream.headers.get("Content-Type") ?? "application/json"

    // For SSE/streaming: pass body stream through directly
    if (upstream.body && (
    ...
    // For non-streaming: buffer, extract usage, record billing
    ...
    } catch (_e) { /* non-JSON response — skip billing */ }
